...
Since the vote_average variable provides some of the most meaningful data, we can explore its distribution and relationship with other variables to better understand viewer preferences and trends.

First, we can explore the distribution of the vote average over all the movies in the dataset. Reminder, the variable is already keeping track of the average rating of each movie, not the average number of votes. 
...
...
Explore relationship between vote avg and genre and budget
...
```{r}
library(tidyverse)
...
ggplot(genre_sep, aes(x=vote_average, y=genres, color = genres)) +
  geom_boxplot() +
